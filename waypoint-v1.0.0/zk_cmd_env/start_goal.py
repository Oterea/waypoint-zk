...
class Goal:
    ...
    def update(self, state):
        ...
        obs_need_scale = np.array([self.bearing_err, self.altitude_err, self.mach_err, self.roll, self.pitch])
        # print(obs_need_scale)
        low = np.array([-np.pi, -4000, -1, -np.pi, -0.5 * np.pi])
        high = np.array([np.pi, 4000, 1, np.pi, 0.5 * np.pi])
